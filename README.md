...
un bonhomme schématique rejoue le mouvement en direct, au tempo.
...
| Fichier | Rôle |
| --- | --- |
| `src/lib/pose.ts` | Modèle du bonhomme : squelette, interpolation, cadrage auto |
| `src/components/Figure.tsx` | Le schéma animé, en boucle au tempo de l'exercice |
...
`y = 125`. Un côté A (devant, tracé plein) et un côté B (derrière, tracé pâle)
suffisent à donner la profondeur. Le cadre de vue est recalculé par exercice à
partir de la boîte englobante de toutes les poses clés : sans ça, un exercice
allongé serait minuscule sur un téléphone.
